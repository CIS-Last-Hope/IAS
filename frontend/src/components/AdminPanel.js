...
function AdminPanel() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8000/admin/admin/course/', {
        headers: {
  ...
  const fetchLessons = async (courseId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:8000/admin/admin/${courseId}/lessons/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleCreateCourse = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `http://localhost:8000/admin/admin/course/create`,
  ...
  const handleDeleteCourse = async (courseTitle) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:8000/admin/admin/course/delete/?title=${courseTitle}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleDeleteLesson = async (courseId, lessonId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:8000/admin/admin/lesson/delete/?course_id=${courseId}&lesson_id=${lessonId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleUpdateCourse = async (courseTitle) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `http://localhost:8000/admin/admin/course/update/?title=${courseTitle}`,
  ...
  const handleUploadLesson = async (courseId) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/admin/admin/lesson/create/?course_id=${courseId}&lesson_id=${nextLessonId}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,

...
function SignUp() {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/auth/sign-up`, userData);
  ...
  const fetchQrCode = async (session_id) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/auth/qr?session_id=${session_id}`, { responseType: 'blob' });
